...
export async function currentUser() {
  ...
  return one(`SELECT u.id, u.email, u.name, u.first_name, u.email_verified_at FROM sessions s JOIN users u ON u.id = s.user_id
...
export async function createEmailVerification(userId) {
  const token = randomToken();
  await q(`INSERT INTO email_verifications (id, user_id, expires_at) VALUES ($1, $2, now() + interval '24 hours')`, [sha256(token), userId]);
  return token;
}
...
// Confirme l'adresse si le lien est valable. Renvoie true si c'est le cas.
export async function confirmEmail(token) {
  const row = await one('SELECT user_id FROM email_verifications WHERE id = $1 AND expires_at > now()', [sha256(String(token))]);
  if (!row) return false;
