...
async function issue(company, invoiceId) {
  ...
  if (doc.doc_type === 'devis') {
    ...
    await q(`UPDATE invoices SET number = $1, issue_date = $2, due_date = $3, status = 'emise', ${SNAPSHOTS}
      WHERE id = $4 AND number IS NULL`, [number, issued, addDays(issued, Number(company.quote_validity) || 30), invoiceId]);
    return;
  }
  await q(`
    WITH seq AS (
    ...
    UPDATE invoices SET
      ...
      issue_date = $4, due_date = $5, status = 'emise', ${SNAPSHOTS}
    FROM seq WHERE invoices.id = $3 AND invoices.number IS NULL`,
  [company.id, year, invoiceId, issued, addDays(issued, Number(company.payment_terms) || 0), numberCode(company, year)]);
}
...
// Copie un document en nouveau brouillon (facture ou devis). La période passe au mois suivant.
export async function duplicate(company, id, { docType } = {}) {
  ...
  return saveDraft(company, {
    ...
    period: type === src.doc_type && src.period ? shiftPeriod(src.period, 1) : src.period,
...
export async function answerQuote(token, accepted) {
  const found = await getInvoiceByToken(token);
  if (!found || !isQuote(found.invoice)) return null;
  const { invoice: inv, company } = found;
  const row = await one(`UPDATE invoices SET status = $1, ${accepted ? 'accepted_at' : 'refused_at'} = now()
    WHERE id = $2 AND status IN ('emise', 'envoyee') RETURNING id`, [accepted ? 'acceptee' : 'refusee', inv.id]);
  if (!row) return inv;
  const owner = await one('SELECT email FROM users WHERE id = $1', [company.owner_id]);
  ...
  return { ...inv, status: accepted ? 'acceptee' : 'refusee' };
}
...
export async function runScheduled() {
  ...
  return { sent: results, reminders: await runReminders() };
}
