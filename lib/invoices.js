...
export async function saveDraft(company, { id, client_id, title, lines, currency, alt_currency, alt_rate,
  vat_rate, withholding_rate, withholding_label, period, notes, send_on, send_tz = '', doc_type = 'facture', source_quote_id = null }) {
  ...
  await q('UPDATE invoices SET send_tz = $1 WHERE id = $2', [send_on ? send_tz : '', invoiceId]);
...
// Annule la facture, émet l'avoir correspondant (AV-FP481-0001) et prévient le client avec le PDF de l'avoir.
...
export async function runScheduled() {
  const due = await q(`SELECT id, company_id FROM invoices
    WHERE (status = 'programmee' AND ${SEND_AT} <= now()) OR status = 'emise'`);
  const results = [];
