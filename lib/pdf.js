...
export async function invoicePdf(inv, company, { credit = false } = {}) {
  ...
  const payItems = await Promise.all(paymentItems(company).map(async (p) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const bottom = doc.page.height - 170;
    ...
    if (mode === 'avoir') {
      ...
      for (const p of payItems) {
        const py = doc.y;
        if (p.img) {
          doc.image(p.img, left, py - 2, { width: 14, height: 14 });
        } else {
          doc.save().roundedRect(left, py - 2, 14, 14, 3).fill(p.b.color).restore();
          doc.font('Helvetica-Bold').fontSize(p.b.short.length > 2 ? 4.6 : 5.6).fillColor(p.b.ink)
            .text(p.b.short, left, py + (p.b.short.length > 2 ? 3 : 2.6), { width: 14, align: 'center', lineBreak: false });
        }
        doc.font('Helvetica').fontSize(10).fillColor(muted).text(`${plain(p.label)} : ${plain(p.value)}`, left + 20, py, { width: width - 20 });
        doc.moveDown(0.25);
      }
      doc.x = left;
    ...
    if (qr) {
      ...
      const qy = doc.page.height - 132;
      doc.moveTo(left, qy - 12).lineTo(right, qy - 12).strokeColor(rule).lineWidth(0.8).stroke().lineWidth(1);
      doc.image(qr, left, qy, { width: 66, height: 66 });
      const tx = left + 80;
      const tw = right - tx - 130;
      doc.font('Helvetica-Bold').fontSize(9).fillColor(ink).text("Vérifier l'authenticité de ce document", tx, qy + 2, { width: tw });
      doc.font('Helvetica').fontSize(8).fillColor(muted)
        .text(`Scannez le QR code, ou saisissez le code sur ${plain(verifyUrl('').replace(/\/v\/$/, '/verifier'))}`, tx, doc.y + 2, { width: tw })
        .text(`Code : ${formatCode(inv.verify_code)}`, tx, doc.y + 3, { width: tw })
