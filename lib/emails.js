...
// En-tête des e-mails d'une entreprise : logo seul s'il existe, sinon nom précédé du logo FactPay
const companyHeading = (company) => (company.logo_key
  ? `<img src="cid:company-logo" alt="${esc(company.name)}" height="48" style="display:block;border:0;height:48px;width:auto;max-width:200px;margin:0 auto">`
  : `<img src="cid:factpay-logo" width="80" height="48" alt="FactPay" style="display:block;border:0;margin:0 auto 10px">${esc(company.name)}`);
const companySub = (company) => [company.address?.replace(/[\r\n]+/g, ', ').replace(/Ð/g, ''), company.legal_ids?.replace(/Ð/g, '')].filter(Boolean).map(esc).join(' · ');
