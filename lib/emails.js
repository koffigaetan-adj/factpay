...
<body style="margin:0;padding:0;background:${C.bg}">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.bg}"><tr><td align="center" style="padding:32px 12px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px">
      <tr><td style="padding:0 4px 16px;font:700 18px ${FONT};color:${C.ink}">${heading}${sub ? `<div style="font:400 13px ${FONT};color:${C.muted};margin-top:2px">${sub}</div>` : ''}</td></tr>
      ...
      <tr><td style="padding:18px 4px 0;font:12px/1.5 ${FONT};color:${C.muted}">${footer || ''}${footer ? '<br>' : ''}<a href="${esc(appUrl())}" style="color:${C.muted};text-decoration:none">Envoyé avec&nbsp;<img src="cid:factpay-logo" width="40" height="24" alt="FactPay" style="border:0;vertical-align:middle"></a></td></tr>
    </table>
  </td></tr></table>
</body></html>`;
}
...
const companyHeading = (company) => `${company.logo_key
  ? `<img src="cid:company-logo" alt="${esc(company.name)}" height="48" style="display:block;border:0;height:48px;width:auto;max-width:200px;margin-bottom:10px">`
  : `<img src="cid:factpay-logo" width="80" height="48" alt="FactPay" style="display:block;border:0;margin-bottom:10px">`}${esc(company.name)}`;
...
export function paymentDeclaredEmail(inv, reference) {
  const cur = inv.currency;
  const link = `${appUrl()}/factures/${inv.id}`;
...
const factpayLogo = () => `<img src="cid:factpay-logo" width="80" height="48" alt="FactPay" style="display:block;border:0">`;
