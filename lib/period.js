...
export function cleanPeriod(raw) {
  ...
  const p = {
    ...
    country: ['FR', 'TG'].includes(raw.country) ? raw.country : 'FR',
  };
  if (p.mode === 'jours') p.dates = workedDays(p);
  return workedDays(p).length ? p : null;
}

// Même période, décalée de n mois (duplication « pour le mois suivant »)
export function shiftPeriod(p, n) {
  const shift = (d) => {
    ...
    return `${target.toISOString().slice(0, 7)}-${String(Math.min(day, last)).padStart(2, '0')}`;
  };
  const shiftMonth = (ym) => (ym ? shift(`${ym}-01`).slice(0, 7) : ym);
  return { ...p, month: shiftMonth(p.month), from: shift(p.from), to: shift(p.to), dates: [...new Set((p.dates || []).map(shift))] };
}
