...
export async function saveFile(file, folder = 'justificatifs') {
  ...
  if (useBlob()) {
    ...
    const acc = access();
    try {
      const blob = await put(name, bytes, { access: acc, contentType: file.type, addRandomSuffix: true });
      return blob.url;
    } catch (err) {
      if (err.message && (err.message.includes('public store') || err.message.includes('Cannot use private access'))) {
