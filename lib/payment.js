...
// Coordonnées bancaires détaillées : intitulé du compte, banque, IBAN/RIB, BIC/SWIFT
export function bankDetails(company) {
  if (!company.iban) return [];
  const details = [];
  if (company.account_holder) details.push(['Intitulé du compte', company.account_holder]);
  if (company.bank_name) details.push(['Banque', company.bank_name]);
  details.push(['IBAN / RIB', company.iban]);
  if (company.bic) details.push(['BIC / SWIFT', company.bic]);
  return details;
}

const bankValue = (company) => [
  company.account_holder && `Intitulé : ${company.account_holder}`,
  company.bank_name && `Banque : ${company.bank_name}`,
  `IBAN : ${company.iban}`,
  company.bic && `BIC : ${company.bic}`,
].filter(Boolean).join(' · ');

// Moyens de paiement avec la banque ou l'opérateur à qui ils appartiennent (pour afficher le logo)
export function paymentItems(company, enabledKeys = null) {
  const items = [];
  if (company.iban) {
    items.push({
      id: 'bank',
      type: 'bank',
      brand: company.bank_name || 'Banque',
      label: `Virement bancaire${company.bank_name ? ` (${company.bank_name})` : ''}`,
      value: bankValue(company),
      details: bankDetails(company),
    });
  }
  const mobiles = parseMobiles(company);
  for (const m of mobiles) {
    const num = formatNumber(m.number, company.country);
    items.push({
      id: `mobile:${m.operator}:${m.number}`,
      type: 'mobile',
      brand: m.operator,
      label: m.operator,
      value: num,
  ...
  if (!mobiles.length && company.mobile_money) {
    items.push({
      id: 'mobile:legacy',
      type: 'mobile',
      brand: 'Mobile Money',
      label: 'Mobile Money',
      value: company.mobile_money,
      details: [['Numéro', company.mobile_money]],
    });
  }
  if (company.spi_alias) {
    items.push({
      id: 'spi',
      type: 'spi',
      brand: 'SPI',
      label: 'Alias SPI (BCEAO)',
      value: company.spi_alias,
      details: [['Alias SPI', company.spi_alias]],
    });
  }
  if (enabledKeys && Array.isArray(enabledKeys)) {
    return items.filter((item) => enabledKeys.includes(item.id));
  }
  return items;
}
...
export function paymentLines(company) {
  const out = [];
  if (company.iban) {
    if (company.account_holder) out.push(['Intitulé du compte', company.account_holder]);
    if (company.bank_name) out.push(['Banque', company.bank_name]);
    out.push(['IBAN / RIB', company.iban]);
    if (company.bic) out.push(['BIC / SWIFT', company.bic]);
  }
  const mobiles = parseMobiles(company);
  for (const m of mobiles) out.push([`Mobile Money (${m.operator})`, formatNumber(m.number, company.country)]);
