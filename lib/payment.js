...
// Coordonnées bancaires telles que sur le RIB : intitulé du compte, IBAN/RIB, BIC
const bankValue = (company) => [
  company.account_holder && `Titulaire : ${company.account_holder}`,
  company.iban,
...
export function paymentItems(company) {
  const items = [];
  if (company.iban) {
    items.push({ brand: company.bank_name || 'Banque', label: `Virement${company.bank_name ? ` (${company.bank_name})` : ''}`,
      value: bankValue(company) });
  }
  const mobiles = parseMobiles(company);
  for (const m of mobiles) items.push({ brand: m.operator, label: m.operator, value: formatNumber(m.number, company.country) });
  if (!mobiles.length && company.mobile_money) items.push({ brand: 'Mobile Money', label: 'Mobile Money', value: company.mobile_money });
  if (company.spi_alias) items.push({ brand: 'SPI', label: 'Alias SPI (BCEAO)', value: company.spi_alias });
  return items;
}
...
export function paymentLines(company) {
  const out = [];
  if (company.iban) {
    out.push([`Virement${company.bank_name ? ` (${company.bank_name})` : ''}`, bankValue(company)]);
  }
  const mobiles = parseMobiles(company);
